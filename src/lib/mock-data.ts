...
function match(values: Omit<Match, "status"> & { status?: Match["status"] }): Match {
  return {
    ...
    futsal_format:
      values.futsal_format ?? ((values.sport_type ?? "futsal") === "futsal" ? "5vs5" : null),
    position_targets: values.position_targets ?? [],
...
export function createMatches(referenceNow?: number): Match[] {
  ...
  return [
    ...
    match({
      ...
      duration_minutes: 120,
      futsal_format: "6vs6",
      position_targets: ["attack", "defense"],
      fee: 9000,
      total_slots: 2,
      remaining_slots: 2,
      min_group_size: 2,
